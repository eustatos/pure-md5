...
if (!isNode) {
  ...
  if (nodeMajor < 15 && !(nodeMajor === 14 && nodeMinor >= 17)) {
    console.log(`Node.js version ${process.versions.node} too low, skipping (need 15+ or 14.17+)`);
  } else {
    describe('WebCryptoBackend in Node.js', () => {
      ...
      beforeAll(async () => {
        ...
        if (!available) {
          ...
          pending('WebCrypto not available');
          return;
        }
        backend = new WebCryptoBackend();
      });
      
      it('should be available in Node.js', async () => {
        const available = WebCryptoBackend.isAvailable();
